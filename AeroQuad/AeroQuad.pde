...
//    tempKinematics.setGyroscope(&gyroSpecific);
...
#ifdef ArduCopter_AQ
  #define LED_Green A13
  #define LED_Red 13
  #define LED_Yellow A15
  #define BUZZER 35
  ...
  /**
   * Put ArduCopter_AQ specific intialization need here
   */
  void initPlatform() {
    pinMode(LED_Red, OUTPUT);
    pinMode(LED_Yellow, OUTPUT);
    ...
    pinMode(A8, OUTPUT); // LED 1
    pinMode(A10, OUTPUT); // LED 2
    pinMode(A12, OUTPUT); // LED 3
    pinMode(A14, OUTPUT); // LED 4
    digitalWrite(A8, LOW); // LED 1 on
    digitalWrite(A10, LOW); // LED 2 on
    digitalWrite(A12, LOW); // LED 3 on
    digitalWrite(A14, LOW); // LED 4 on
  ...
  void initPlatformEEPROM(void) {
    ...
    PID[BARO_ALTITUDE_HOLD_PID_IDX].P = 10.0;
    PID[BARO_ALTITUDE_HOLD_PID_IDX].I = 0.5;
    PID[BARO_ALTITUDE_HOLD_PID_IDX].D = 0.0;
    PID[BARO_ALTITUDE_HOLD_PID_IDX].windupGuard = 15.0; //this prevents the 0.1 I term to rise too far
 }
  ...
  /*  PID[HEADING_HOLD_PID_IDX].P = 3.0;
    PID[HEADING_HOLD_PID_IDX].I = 0.1;
    PID[HEADING_HOLD_PID_IDX].D = 0.0;  */
    PID[ATTITUDE_GYRO_XAXIS_PID_IDX].P = PID[RATE_XAXIS_PID_IDX].P - 10.0;
    PID[ATTITUDE_GYRO_XAXIS_PID_IDX].I = PID[RATE_XAXIS_PID_IDX].I ;
    PID[ATTITUDE_GYRO_XAXIS_PID_IDX].D = PID[RATE_XAXIS_PID_IDX].D;
    PID[ATTITUDE_GYRO_YAXIS_PID_IDX].P = PID[RATE_YAXIS_PID_IDX].P - 10.0;
    PID[ATTITUDE_GYRO_YAXIS_PID_IDX].I = PID[RATE_YAXIS_PID_IDX].I;
    PID[ATTITUDE_GYRO_YAXIS_PID_IDX].D = PID[RATE_YAXIS_PID_IDX].D; 
  }
